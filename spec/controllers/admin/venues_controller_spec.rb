require 'rails_helper'

RSpec.describe Admin::VenuesController, :type => :controller do
  include_context 'session'
  # This should return the minimal set of attributes required to create a valid
  # Venue. As you add validations to Venue, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    attributes_for(:venue)
  }

  let(:invalid_attributes) {
    { name: nil }
  }

  describe "GET index" do
    it "assigns all venues as @venues" do
      venue = create(:venue)
      get :index, {}, valid_admin_session
      expect(assigns(:venues)).to include venue
    end
  end

  describe "GET show" do
    it "assigns the requested venue as @venue" do
      venue = create(:venue)
      get :show, {:id => venue.id }, valid_admin_session
      expect(assigns(:venue)).to eq(venue)
    end
  end

  describe "GET new" do
    it "assigns a new venue as @venue" do
      get :new, {}, valid_admin_session
      expect(assigns(:venue)).to be_a_new(Venue)
    end
  end

  describe "GET edit" do
    it "assigns the requested venue as @venue" do
      venue = create(:venue)
      get :edit, {:id => venue.to_param}, valid_admin_session
      expect(assigns(:venue)).to eq(venue)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Venue" do
        expect {
          post :create, {:venue => valid_attributes}, valid_admin_session
        }.to change(Venue, :count).by(1)
      end

      it "assigns a newly created venue as @venue" do
        post :create, {:venue => valid_attributes}, valid_admin_session
        expect(assigns(:venue)).to be_a(Venue)
        expect(assigns(:venue)).to be_persisted
      end

      it "redirects to the created venue" do
        post :create, {:venue => valid_attributes}, valid_admin_session
        expect(response).to redirect_to([:admin, Venue.last])
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved venue as @venue" do
        post :create, {:venue => invalid_attributes}, valid_admin_session
        expect(assigns(:venue)).to be_a_new(Venue)
      end

      it "re-renders the 'new' template" do
        post :create, {:venue => invalid_attributes}, valid_admin_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      let(:new_attributes) {
        { name: 'Some new name' }
      }

      it "updates the requested venue" do
        venue = Venue.create! valid_attributes
        put :update, {:id => venue.to_param, :venue => new_attributes}, valid_admin_session
        venue.reload
        expect(venue.name).to eq('Some new name')
      end

      it "assigns the requested venue as @venue" do
        venue = Venue.create! valid_attributes
        put :update, {:id => venue.to_param, :venue => valid_attributes}, valid_admin_session
        expect(assigns(:venue)).to eq(venue)
      end

      it "redirects to the venue" do
        venue = Venue.create! valid_attributes
        put :update, {:id => venue.to_param, :venue => valid_attributes}, valid_admin_session
        expect(response).to redirect_to([:admin, venue])
      end
    end

    describe "with invalid params" do
      it "assigns the venue as @venue" do
        venue = Venue.create! valid_attributes
        put :update, {:id => venue.to_param, :venue => invalid_attributes}, valid_admin_session
        expect(assigns(:venue)).to eq(venue)
      end

      it "re-renders the 'edit' template" do
        venue = Venue.create! valid_attributes
        put :update, {:id => venue.to_param, :venue => invalid_attributes}, valid_admin_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested venue" do
      venue = create(:venue)
      expect {
        delete :destroy, {:id => venue.to_param}, valid_admin_session
      }.to change(Venue, :count).by(-1)
    end

    it "redirects to the venues list" do
      venue = create(:venue)
      delete :destroy, {:id => venue.to_param}, valid_admin_session
      expect(response).to redirect_to(venues_url)
    end
  end

end
